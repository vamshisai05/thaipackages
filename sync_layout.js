...
const drawerMenu = `<ul class="drawer-menu">
...
const footer = `<footer class="site-footer" id="contact">
    <div class="container">
...
for (const file of files) {
  ...
  const getNavClass = (link) => file === link ? 'nav-link active' : 'nav-link';
  
  const navLinks = `<ul class="nav-links">
          <li><a href="packages.html" class="${getNavClass('packages.html')}">Packages</a></li>
          <li><a href="destinations.html" class="${getNavClass('destinations.html')}">Destinations</a></li>
          <li><a href="about.html" class="${getNavClass('about.html')}">About</a></li>
          <li><a href="contact.html" class="${getNavClass('contact.html')}">Contact</a></li>
        </ul>`;

  content = content.replace(/<ul class="nav-links">[\s\S]*?<\/ul>/, navLinks);
  content = content.replace(/<ul class="drawer-menu">[\s\S]*?<\/ul>/, drawerMenu);
  content = content.replace(/<footer class="site-footer"[\s\S]*?<\/footer>/, footer);

  fs.writeFileSync(file, content, 'utf8');
}
console.log('Updated HTML files via node.js');
